...
uint32_t SPI_ReceiveData(SPI_TypeDef* SPIx)
{
	...
	temp |=(u32)SPIx->RXREG;
	if(SPIx->EXTSCR>8||SPIx->EXTSCR==0)    temp |=(u32) (SPIx->RXREG)<<8;
	if(SPIx->EXTSCR>16||SPIx->EXTSCR==0)  	temp |=(u32)( SPIx->RXREG)<<16;
	if(SPIx->EXTSCR>24||SPIx->EXTSCR==0)	temp |=(u32)( SPIx->RXREG)<<24;

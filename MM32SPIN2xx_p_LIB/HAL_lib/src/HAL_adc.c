...
#define ADCHS_TSVREFE_Set            				((uint32_t)0x0000000C)
#define ADCHS_TSVREFE_Reset            				((uint32_t)0xFFFFFFF3)
...
void ADC_RegularChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
    ...
    break;
    case ADC_Channel_8: ADCx->ADCHS |= CHEN8_ENABLE;  
    break;
    case ADC_Channel_9: ADCx->ADCHS |= CHEN9_ENABLE;  
    break;
    case ADC_Channel_10: ADCx->ADCHS |= CHEN10_ENABLE;
    break;
    case ADC_Channel_11: ADCx->ADCHS |= CHEN11_ENABLE;  
    break;
    case ADC_Channel_14: ADCx->ADCHS |= CHEN14_ENABLE;  
    break;
    case ADC_Channel_15: ADCx->ADCHS |= CHEN15_ENABLE;  
    break;
    case ADC_Channel_All:ADCx->ADCHS |= CHALL_ENABLE;
    break;
    default:
...
void ADC_TempSensorVrefintCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        ADC1->ADCFG |= ADCHS_TSVREFE_Set ;
        
    }
    else
    {
        /* Disable the temperature sensor and Vrefint channel*/
        ADC1->ADCFG &= ADCHS_TSVREFE_Reset;
